...
std::string format_line(const char* fname, unsigned lineno, int verbosity_int, const std::string& message) {
    const char* thread = g_thread_name;
    if (thread[0] != '\0') {
        return fmt::format("[{}] [{}] {}:{} {}", level_to_string(verbosity_int), thread,
            basename_from_path(fname), lineno, message);
    }
    return fmt::format("[{}] {}:{} {}", level_to_string(verbosity_int),
        basename_from_path(fname), lineno, message);
}
...
void NativeBackend::log(int verbosity, const char* fname, unsigned int lineno, const char* message) {
    if (message == nullptr || (message[0] == '\0' && verbosity != static_cast<int>(logger_verbosity_enum::VERBOSITY_FATAL))) {
        return;
    }
    ...
    std::vector<callback_entry> callbacks_copy;
    {
        ...
        if (g_console_mode.load(std::memory_order_relaxed) || verbosity == static_cast<int>(logger_verbosity_enum::VERBOSITY_FATAL)) {
...
void NativeBackend::log_to_file(const char* path, bool truncate, int verbosity) {
    ...
    sink.path = path;
    sink.verbosity = verbosity;
    const auto open_mode = truncate ? (std::ios::out | std::ios::trunc) : (std::ios::out | std::ios::app);
...
void NativeBackend::add_callback(const char* id,
    ...
    const std::scoped_lock guard(g_io_mutex);
    g_callbacks[id] = callback_entry{log_handler_ptr, on_close_ptr, on_flush_ptr, user_data, verbosity};
}
